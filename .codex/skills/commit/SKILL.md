...
`build`, `ci`, and `revert`. Use a stable lowercase scope such as `app`,
`docs`, `ci`, or `repo`. Do not use `impl`, an empty scope, or an unscoped
prefix. Do not add a space after the colon. The Chinese description must be a
concise Simplified Chinese verb-object phrase; do not use an English
description.
